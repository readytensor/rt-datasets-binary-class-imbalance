...
def load_metadata(dataset_cfg_path: str) -> pd.DataFrame:
    ...
    for col in dataset_metadata.columns:
        dataset_metadata[col] = dataset_metadata[col].apply(strip_quotes)
...
def save_json(data: Dict[str, Any], output_fpath: str):
    """
    Save data as JSON.
    ...
    """
    with open(output_fpath, "w", encoding="utf-8") as f_h:
        json.dump(data, f_h, cls=JSONEncoder, indent=2)
